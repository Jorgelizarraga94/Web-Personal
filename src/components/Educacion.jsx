  ...
  return (
    <section className="py-4">
      <div className="container" style={{ maxWidth: '900px' }}>
        <h3 className="tech-stack-title text-center mb-4 mt-2 fw-bold fs-3 text-light opacity-75 mb-2">Educación y Formación</h3>
        
        {/* Carrera principal (Destacada arriba en ancho completo) */}
        <div className="mb-4">
          {universityEducation.map((item, index) => (
            <div key={index} className="education-card p-4 rounded position-relative">
              <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-1">
                <h5 className="text-light fw-bold mb-0">{item.degree}</h5>
                <span className="badge bg-dark border border-secondary text-pink">{item.period}</span>
              </div>
        ...
        {/* Cursos y especializaciones en grilla de 2 columnas */}
              ...
              <div className="education-card p-3 rounded h-100 d-flex flex-column justify-content-between">
                <div>
                  <div className="d-flex justify-content-between align-items-start gap-2 mb-1">
                    ...
                    <span className="badge bg-dark border border-secondary text-pink small">{item.period}</span>
                  </div>
